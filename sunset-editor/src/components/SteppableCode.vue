...
onMounted(() => {
    ...
    const code_lines = container_el.querySelectorAll('.code-line')
    ...
    if (props.steppable_lines)
    {
        ...
        for (let i = 0; i < code_steps.length; ++i)
        {
            ...
            code_steps[i].addEventListener('mouseenter', (_: MouseEvent) => {
                if (lines.length > i) {
                    code_lines[lines[i] - 1].classList.add('highlighted')
                    code_lines[lines[i] - 1].scrollIntoView({ behavior: 'smooth', block: 'nearest' })
                }
            })
            code_steps[i].addEventListener('mouseleave', (_: MouseEvent) => {
                if (lines.length > i) {
                    code_lines[lines[i] - 1].classList.remove('highlighted')
                }
            })
        }
    }
    ...
    if (props.removed_lines)
    {
        ...
        for (let i = 0; i < code_nukes.length; ++i)
        {
            ...
            code_nukes[i].addEventListener('mouseenter', (_: MouseEvent) => {
                if (lines.length > i) {
                    code_lines[lines[i] - 1].classList.add('highlighted')
                }
            })
            code_nukes[i].addEventListener('mouseleave', (_: MouseEvent) => {
                if (lines.length > i) {
                    code_lines[lines[i] - 1].classList.remove('highlighted')
                }
            })
...
.locatable-code :deep(.code-step), .locatable-code :deep(.code-nuke) {
    position: relative;
...
.locatable-code :deep(.code-step i), .locatable-code :deep(.code-nuke i) {
    position: absolute;
...
.locatable-code :deep(.code-step p), .locatable-code :deep(.code-nuke p) {
    padding: 5px;
}
